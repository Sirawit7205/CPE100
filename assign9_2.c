...
int secondFiltering(char filterInput[], char separatedStr[][100]);
int findStrType(char input[]);
void displayResult(int strCount,char separatedStr[][100],int strTypeCnt[],int strTypeList[]);
...
int main()
{
	char rawInput[1005]={},filterInput[1000]={},separatedStr[100][100]={};
	...
	do
	{
		...
		//String Token + Count
		strCount=secondFiltering(filterInput,separatedStr);
		...
		if(exitMode==0)
		{
			...
			//Print output
			displayResult(strCount,separatedStr,strTypeCnt,strTypeList);
		}
...
int secondFiltering(char filterInput[], char separatedStr[][100])
{
	...
	while(token!=NULL)
	{
		if(token!=NULL)
		{
			strcpy(separatedStr[count],token);
			count++;
		}
		token=strtok(NULL," ");
	}
